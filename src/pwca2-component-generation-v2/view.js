...
const buildRequiredQuestionsCard = ( state ) => {
    ...
    return (
        <div>
            {(requiredQuestions.length != 0 && !datasheet) ? (
                ...
                {questions.map((question) => {
                    ...
                    if (readOnly || datasheet) {
                        ...
                        if ((question.type == "now-typeahead-multi-choice" || question.type == "now-typeahead-multi" || question.type == "now-typeahead") && question.value != "") {
                            ...
                            questionValue = question.properties.placeholder;
                        } else if (question.type == "now-dropdown" || question.type == "now-dropdown:multi" || question.type == "now-dropdown:multi" || question.type == "now-radio-buttons" || question.type == "now-select") {
                                        ...
                                        <div className="flex-column-container">
                                            ...
                                            <div>
                                                {question.properties.type == "date" ? (
                                                    <now-input ref={question.ref} className="halfWidth" component-id={question.id} id={question.id} readonly={disabled || question.readOnly} size={question.properties.size} placeholder={question.properties.placeholder} step="any" type={question.properties.type} messages={[{"status":question.message.status,"icon":question.message.icon,"content":question.message.content}]} value={question.value}></now-input> 
                                                ) : (
                                                    <now-input ref={question.ref} className="halfWidth" component-id={question.id} id={question.id} readonly={disabled || question.readOnly} size={question.properties.size} placeholder={question.properties.placeholder} step="any" type={question.properties.type} messages={[{"status":question.message.status,"icon":question.message.icon,"content":question.message.content}]} value={question.value}></now-input> 
                                                )}   
                                            </div>                                                                                     
